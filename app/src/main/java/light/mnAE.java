...
public class mnAE {
	...
	private static String aeName = "mnAE";
	private static String cntName = "containers_group";

	private static String aeProtocol = "http";
	private static String aeIp = "127.0.0.1";
	private static int aePort = 1603;
...
// discovery
		HttpResponse httpResponse = RestHttpClient.get( originator,
				csePoa + "/~/" + cseId + "/" + cseName + "?fu=1&ty=3&drt=2");
...
// ty=9 
		...
		obj.put("rn", cntName);
		obj.put("mt", 3);
		obj.put("mid", uril_arr);
		obj.put("mnm", 10);
		grp = new JSONObject();
		grp.put("m2m:grp", obj);
		RestHttpClient.post(originator, csePoa + "/~/" + cseId + "/" + cseName
				+ "/" + aeName, grp.toString(), 9);
