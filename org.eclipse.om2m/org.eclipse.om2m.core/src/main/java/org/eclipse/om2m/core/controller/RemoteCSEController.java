...
public class RemoteCSEController extends Controller {
	...
	public ResponsePrimitive doCreate(RequestPrimitive request) {
		 ...
		 * 
		 * cseType O poa O CseBase M CSE-ID M M2M-EXT-ID O Trigger-Recipient-ID
		 * O requestReachability M nodeLink NP
		 * 
		 * descendantCSEs adding
		 */
		...
		DAO<ResourceEntity> dao = (DAO<ResourceEntity>) patterns.getDAO(
				request.getTo(), dbs);
		if (dao == null) {
			throw new ResourceNotFoundException("Cannot find parent resource");
		}
		// get the parent entity
		ResourceEntity parentEntity = (ResourceEntity) dao.find(transaction,
				request.getTo());
		// check the parent existence
		if (parentEntity == null) {
			throw new ResourceNotFoundException(
					"Cannot find the parent resource");
		}
		...
		if (request.getContent() == null) {
			throw new BadRequestException(
					"A content is requiered for RemoteCSE creation");
		}
		...
		try {
			if (request.getRequestContentType().equals(MimeMediaType.OBJ)) {
				...
				remoteCse = (RemoteCSE) DataMapperSelector.getDataMapperList()
						.get(request.getRequestContentType())
		...
		// creationTime NP
		// labels O
		ControllerUtil.CreateUtil.fillEntityFromGenericResource(remoteCse,
				remoteCseEntity);
		...
		if (remoteCse.isRequestReachability() == null) {
			...
			remoteCseEntity.setRequestReachability(remoteCse
					.isRequestReachability());
		}
		// accessControlPolicyIDs O
		if (!remoteCse.getAccessControlPolicyIDs().isEmpty()) {
			remoteCseEntity.setAccessControlPolicies(ControllerUtil
					.buildAcpEntityList(remoteCse.getAccessControlPolicyIDs(),
							transaction));
		} else {
		...
		if (!remoteCse.getDynamicAuthorizationConsultationIDs().isEmpty()) {
			remoteCseEntity.setDynamicAuthorizationConsultations(ControllerUtil
					.buildDacEntityList(
							remoteCse.getDynamicAuthorizationConsultationIDs(),
							transaction));
		}
		// expiration time O
		...
		if (!remoteCse.getAnnouncedAttribute().isEmpty()) {
			remoteCseEntity.getAnnouncedAttribute().addAll(
					remoteCse.getAnnouncedAttribute());
		}
		// cseType O
		...
		if (!remoteCse.getPointOfAccess().isEmpty()) {
			remoteCseEntity.getPointOfAccess().addAll(
					remoteCse.getPointOfAccess());
		}
		...
		if (remoteCse.getTriggerRecipientID() != null) {
			remoteCseEntity.setTriggerRecipientID(remoteCse
					.getTriggerRecipientID());
		}

		// descendant CSE O
		if (!remoteCse.getDescendantCSEs().isEmpty()) {
			remoteCseEntity.getDescendantCSEs().addAll(
					remoteCse.getDescendantCSEs());
		}

		// Thực hiện update thông tin lên cho cha
		if (!Constants.CSE_TYPE.equalsIgnoreCase(CSEType.IN)) {
			RequestPrimitive requestUpdate = new RequestPrimitive();
			// String remotePoa = null;
			String remotePoa = "http://" + Constants.REMOTE_CSE_IP + ":"
					+ Constants.REMOTE_CSE_PORT + "/~"
					+ Constants.REMOTE_CSE_CONTEXT;
			if (Constants.REMOTE_CSE_CONTEXT.length() > 1
					&& !Constants.REMOTE_CSE_CONTEXT.equals("/~")) {
				remotePoa += "/";
			}
			remotePoa += Constants.REMOTE_CSE_ID + "/"
					+ Constants.REMOTE_CSE_NAME + "/" + Constants.CSE_NAME;
			LOGGER.info(" [dc] address to update descendantCSEs \n" + remotePoa);
			RemoteCSE remoteCSE = new RemoteCSE();
			String contentFormat = System.getProperty(
					"org.eclipse.om2m.registration.contentFormat",
					MimeMediaType.XML);
			remoteCSE.getDescendantCSEs().add(remoteCse.getCSEID());
			String representation = DataMapperSelector.getDataMapperList()
					.get(contentFormat).objToString(remoteCSE);

			requestUpdate.setFrom(Constants.ADMIN_REQUESTING_ENTITY);
			requestUpdate.setTo(remotePoa);
			requestUpdate.setOperation(Operation.UPDATE);
			requestUpdate.setContent(representation);
			requestUpdate.setResourceType(BigInteger
					.valueOf(ResourceType.REMOTE_CSE));
			requestUpdate.setRequestContentType(contentFormat);
			requestUpdate.setRequestIdentifier("001");

			ResponsePrimitive responseUpdate = RestClient
					.sendRequest(requestUpdate);
			LOGGER.info("[dc] DONE Update in Create + \n" + responseUpdate.toString());

		}

		// creating the corresponding entity
		String generatedId = generateId("", "");
		remoteCseEntity.setResourceID("/" + Constants.CSE_ID + "/"
				+ ShortName.REMOTE_CSE + Constants.PREFIX_SEPERATOR
				+ generatedId);
		// set name if present and without any conflict
		if (remoteCse.getName() != null) {
			if (!patterns.checkResourceName(remoteCse.getName())) {
				throw new BadRequestException(
						"Name provided is incorrect. Must be:"
								+ patterns.ID_STRING);
			}
		...
		remoteCseEntity.setHierarchicalURI(parentEntity.getHierarchicalURI()
				+ "/" + remoteCseEntity.getName());
		if (!UriMapper.addNewUri(remoteCseEntity.getHierarchicalURI(),
		...
		if (newOriginator) {
			...
			acpEntity
					.setResourceID("/" + Constants.CSE_ID + "/" + ShortName.ACP
							+ Constants.PREFIX_SEPERATOR + generateId());
			acpEntity.setName(ShortName.ACP + ShortName.REMOTE_CSE
			...
			ruleEntity.setNotify(true);
			ruleEntity.setDiscovery(true);
			ruleEntity.getAccessControlOriginators().add(
					new AccessControlOriginatorEntity(request.getFrom()));
			ruleEntity.getAccessControlOriginators().add(
					new AccessControlOriginatorEntity(
							Constants.ADMIN_REQUESTING_ENTITY));
			acpEntity.getPrivileges().add(ruleEntity);
			acpEntity.setHierarchicalURI("/" + Constants.CSE_ID + "/"
					+ acpEntity.getName());
			UriMapper.addNewUri(acpEntity.getHierarchicalURI(),
					acpEntity.getResourceID(),
					ResourceType.ACCESS_CONTROL_POLICY);
			dbs.getDAOFactory().getAccessControlPolicyDAO()
					.create(transaction, acpEntity);

			AccessControlPolicyEntity acpDB = dbs.getDAOFactory()
					.getAccessControlPolicyDAO()
					.find(transaction, acpEntity.getResourceID());
			CSEBaseEntity cseBase = dbs.getDAOFactory().getCSEBaseDAO()
					.find(transaction, "/" + Constants.CSE_ID);
		...
		dbs.getDAOFactory().getRemoteCSEDAO()
				.create(transaction, remoteCseEntity);

		// Get the managed object from db
		RemoteCSEEntity csrDB = dbs.getDAOFactory().getRemoteCSEDAO()
				.find(transaction, remoteCseEntity.getResourceID());
		...
		for (DynamicAuthorizationConsultationEntity dace : csrDB
				.getDynamicAuthorizationConsultations()) {
			DynamicAuthorizationConsultationEntity daceFromDB = dbs
					.getDAOFactory().getDynamicAuthorizationDAO()
					.find(transaction, dace.getResourceID());
			daceFromDB.getLinkedRemoteCSEEntities().add(csrDB);
			dbs.getDAOFactory().getDynamicAuthorizationDAO()
					.update(transaction, daceFromDB);
		}
	...
	public ResponsePrimitive doRetrieve(RequestPrimitive request) {
		...
		RemoteCSEEntity csrEntity = dbs.getDAOFactory().getRemoteCSEDAO()
		...
		// retrieve
		List<AccessControlPolicyEntity> acpList = csrEntity
		...
		RemoteCSE csr = EntityMapperFactory.getRemoteCseMapper()
				.mapEntityToResource(csrEntity, request);
	...
	public ResponsePrimitive doUpdate(RequestPrimitive request) {
		 ...
		 * @resourceName NP resourceType NP resourceID NP parentID NP
		 * accessControlPolicyIDs O creationTime NP expirationTime O
		 * lastModifiedTime NP labels NP announceTo O announcedAttribute O
		 * 
		 * cseType NP poa O CseBase NP CSE-ID NP M2M-EXT-ID O
		 * Trigger-Recipient-ID O requestReachability O nodeLink NP
		 */
		...
		// check ACP
		checkACP(csrEntity.getAccessControlPolicies(), request.getFrom(),
				Operation.UPDATE);

		// check if content is present
		if (request.getContent() == null) {
			throw new BadRequestException(
					"A content is requiered for RemoteCSE Update");
		}
		...
		try {
			...
			throw new BadRequestException(
					"Incorrect resource representation in content", e);
		}
		if (csr == null) {
			throw new BadRequestException("Error in provided content");
		}
		...
		if (!csr.getAccessControlPolicyIDs().isEmpty()) {
			for (AccessControlPolicyEntity acpe : csrEntity
					.getAccessControlPolicies()) {
				checkSelfACP(acpe, request.getFrom(), Operation.UPDATE);
			}
			csrEntity.setAccessControlPolicies(ControllerUtil
					.buildAcpEntityList(csr.getAccessControlPolicyIDs(),
							transaction));
			modifiedAttributes.getAccessControlPolicyIDs().addAll(
					csr.getAccessControlPolicyIDs());
		}

		// dynamicAuthorizationConsultationIDs O
		if (!csr.getDynamicAuthorizationConsultationIDs().isEmpty()) {
			csrEntity.setDynamicAuthorizationConsultations(ControllerUtil
					.buildDacEntityList(
							csr.getDynamicAuthorizationConsultationIDs(),
							transaction));

			// update link with remoteCseEntity - DacEntity
			for (DynamicAuthorizationConsultationEntity dace : csrEntity
					.getDynamicAuthorizationConsultations()) {
				DynamicAuthorizationConsultationEntity daceFromDB = dbs
						.getDAOFactory().getDynamicAuthorizationDAO()
						.find(transaction, dace.getResourceID());
		...
		if (!csr.getAnnouncedAttribute().isEmpty()) {
			...
			csrEntity.getAnnouncedAttribute().addAll(
					csr.getAnnouncedAttribute());
			modifiedAttributes.getAnnouncedAttribute().addAll(
					csr.getAnnouncedAttribute());
		}
		// poa O
		if (!csr.getPointOfAccess().isEmpty()) {
			...
			modifiedAttributes.getPointOfAccess()
					.addAll(csr.getPointOfAccess());
		}
		// M2M-EXT-ID O
		...
		if (csr.getTriggerRecipientID() != null) {
			...
			modifiedAttributes.setTriggerRecipientID(csr
					.getTriggerRecipientID());
		}
		// requestReachability O
		if (csr.isRequestReachability() != null) {
			csrEntity.setRequestReachability(csr.isRequestReachability());
			modifiedAttributes.setRequestReachability(csr
					.isRequestReachability());
		}

		// dc
		if (!csr.getDescendantCSEs().isEmpty()) {
			//csrEntity.getDescendantCSEs().clear();
			csrEntity.getDescendantCSEs().addAll(csr.getDescendantCSEs());
			modifiedAttributes.getDescendantCSEs().addAll(
					csr.getDescendantCSEs());
		}
		
//		for(int i=0;i<csrEntity.getDescendantCSEs().size();i++){
//		   LOGGER.info("[dc] List dc:\n" + csrEntity.getDescendantCSEs().get(i));
//		} 		
		...
		if (!Constants.CSE_TYPE.equalsIgnoreCase(CSEType.IN)) {
			...
			String contentFormat = System.getProperty(
					"org.eclipse.om2m.registration.contentFormat",
					MimeMediaType.XML);
			remoteCSE.getDescendantCSEs().addAll(csr.getDescendantCSEs());
			String representation = DataMapperSelector.getDataMapperList()
					.get(contentFormat).objToString(remoteCSE);
			...
			ResponsePrimitive responseUpdate = RestClient
					.sendRequest(requestUpdate);
			LOGGER.info("[dc] DONE update in update + \n" + responseUpdate.toString());
		...
		Notifier.notify(csrEntity.getSubscriptions(), csrEntity,
	...
	public ResponsePrimitive doDelete(RequestPrimitive request) {
		...
		RemoteCseService.getInstance().removeRemoteCseAndPublish(
				csrEntity.getName());
