...
public class Mapper implements DataMapperService {
	...
	// org.eclipse.om2m.commons.resource:____
	// hieuttbk: các object java trong packe ...resource này sẽ được đem đi mapping XML
	//hieuttbk10-07. Ignore all flexcontainer
		...
		try {
			if(context==null){
				if(mediaType.equals(MimeMediaType.JSON)){
					...
					InputStream iStreamJsonBinding = classLoader.getResourceAsStream("json-binding.json");// ghi input vào file .json
					...
					properties.put("eclipselink.media-type", "application/json");// JAXB using persistent lib could set meida-type JSON ---MOXy
	...
	public String objToString(Object obj) {
		try {
			...
			if (obj instanceof URIList) {	// obj có phải kiểu dữ liệu URIList không ? 
